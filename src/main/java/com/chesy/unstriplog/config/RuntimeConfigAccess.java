...
public final class RuntimeConfigAccess {
    ...
    public static boolean allowUnknownLog() {
        ...
        return UnstripLogConfig.allowUnknownLog();
    }

    public static Item barkItem() {
        if (useSyncedSnapshot()) {
            return BuiltInRegistries.ITEM.getValue(Identifier.parse(SyncedConfigState.barkItemId()));
        }
        ...
        return BuiltInRegistries.ITEM.getValue(Identifier.parse(UnstripLogConfig.getConfiguredItemId()));
    }
    ...
    private static boolean useSyncedSnapshot() {
        return SyncedConfigState.hasSnapshot() && FabricLoader.getInstance().getEnvironmentType() == EnvType.CLIENT;
    }
