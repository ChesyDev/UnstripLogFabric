...
public class UnstripDetailedConfig {
    ...
    public static void init() {
        configPath = FabricLoader.getInstance().getConfigDir().resolve("unstriplog/unstrip-detailed.json");
    ...
    public static void load() {
        ...
        if (UnstripLog.SERVER != null) {
            ConfigSyncManager.syncAll(UnstripLog.SERVER);
        }
    }
        ...
        LogHandler.STRIPPED_LOG.forEach((key, value) -> {
            if (LogHandler.BARK_TYPE.get(value) != null){
                ENTRIES.add(new LogEntry(
                        value,
                        key,
                        new ItemEntry(ModItems.BARK.builtInRegistryHolder(), DataComponentPatch.builder().set(ModDataComponents.BARK_TYPE, BarkTypeConfig.getByBase(LogHandler.BARK_TYPE.get(value))).build()),
                        Optional.empty()
                ));
            }
        });
